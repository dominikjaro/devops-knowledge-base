...
![Types of Hypervisors](type_of_hypervisers.png)
...
* `/opt` 3rd party programs you install that DONT split its components (optional or add-on software)
* `/boot` files required for booting
* `/etc` (editable text config) system config
* `/dev` device files/hardware or driver files (webcam, mouse, keyboard etc)
* `/var` stores logs
* `/var/cache`
...
* `cat /etc/shadow` = show password info (only root can access)
* `lscpu` = cpu info
* `lsmem` = memory info
...
* `du -sh `= show size of file or dir in human readable format
